...
采用递推公式计算每个历元 i 的**平均宽巷模糊度以及方差**：
$$
\begin{align}
\text{sum}_{N_{\text{WL}}}(i) &= \text{sum}_{N_{\text{WL}}}(i-1) + N_{\text{WL}}(i) \tag{3a} \\
\text{sum}_{N_{\text{WL}}^2}(i) &= \text{sum}_{N_{\text{WL}}^2}(i-1) + N_{\text{WL}}^2(i) \tag{3b} \\
\overline{N}_{\text{WL}}(i) &= \frac{\text{sum}_{N_{\text{WL}}}(i)}{i} \tag{3c} \\
\sigma^2(i) &= \frac{\text{sum}_{N_{\text{WL}}^2}(i)}{i} - \left(\overline{N}_{\text{WL}}(i)\right)^2 \tag{4}
\end{align}
$$

$$
式中：\overline{N}_{\text{WL}}(i) 为前 i 个历元宽巷模糊度的平均值；\sigma^2(i) 表示前 i 个历元的方差。
$$
...
**1. 样本数量要求**：
$$
i \geq N_{\text{min}} \tag{5a} \\
其中 N_{\text{min}} = 5 为最小样本数要求。
$$


**2. 周跳判定条件**： 当样本数量满足要求时，若同时满足式（5b）中的条件，则认为当前历元存在周跳：
$$
\left. \begin{matrix} 
\vert N_{\text{WL}}(i) - \overline{N}_{\text{WL}}(i-1) \vert \geqslant 3\sigma \\
\text{且} \quad \sigma(i-1) > 10^{-10} \\
\text{且} \quad \vert N_{\text{WL}}(i) - N_{\text{WL}}(i+1) \vert \leqslant 1 \quad \text{(当} \, i < n \text{时)}
\end{matrix} \right\} \tag{5b}
$$
**3. 特殊情况处理**：
$$
当 i < N_{\text{min}} 时，所有历元标记为正常 \\
对于最后一个历元（i = n），无法检查与下一历元的一致性，仅依据偏差大小判定\\
发生周跳后，统计量从当前历元开始重新累积，防止周跳影响后续检测\\
$$
...
2. 累积统计量：\text{sum}*{N*{\text{WL}}} 和 \text{sum}*{N*{\text{WL}}^2} \\

3. 当样本数达到最小要求时，开始周跳检测 \\

4. 若检测到周跳，重置统计量从当前历元开始重新累积 \\

5. 若未检测到周跳，继续累积统计量用于下一次检测 \\
$$
这种方法相比原始递推公式更加稳定，避免了样本数量不足时的误判，并且通过重置统计量的方式，确保周跳不会影响后续历元的检测精度。
